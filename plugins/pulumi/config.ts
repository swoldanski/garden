...
import { DeepPrimitiveMap, joi, joiSparseArray, joiStringMap } from "@garden-io/core/build/src/config/common"
...
import { varfileDescription } from "@garden-io/core/build/src/config/project"
import { GardenModule } from "@garden-io/sdk/types"
import { dedent } from "@garden-io/sdk/util/string"
import { supportedVersions, defaultPulumiVersion } from "./cli"
...
export const pulumiProviderConfigSchema = providerConfigBaseSchema()
  .keys({
    ...
    version: joi
      ...
      `),
    previewDir: joi.posixPath().subPathOnly().description(
      dedent`
...
const variablesSchema = () => joiStringMap(joi.any())
...
export const pulumiModuleSchema = () =>
  joi.object().keys({
    ...
    `),
    pulumiVariables: variablesSchema().default({}).description(dedent`
    ...
    `),
    pulumiVarfiles: joiSparseArray(joi.posixPath())
      .description(
        dedent`
    ...
    cacheStatus: joi.boolean().default(false).description(
      dedent`
    ...
    `),
    deployFromPreview: joi.boolean().default(false).description(
      dedent`
    ...
    stack: joi.string().allow(null).description("The name of the pulumi stack to use. Defaults to the module name."),
  })
