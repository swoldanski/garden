...
# A map of config variables to use when applying the stack, overriding variables set in the `Pulumi.<stack>.yaml` file
# (if any).
#
...
# Specify one or more paths (relative to the module root) to files containing pulumi config variables. If
...
A map of config variables to use when applying the stack, overriding variables set in the `Pulumi.<stack>.yaml` file (if any).
